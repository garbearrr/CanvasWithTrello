...
def sync_once(
    *,
    ...
    try:
        ...
        synckey_to_card = {}
    logger.info(
        "Sync start: board_id=%s due_within_days=%s state_items=%s recovered_synckeys=%s",
        board_id,
        due_within_days,
    ...
    for course in courses:
        ...
        for item in canvas.upcoming_items(course_id, within_days=due_within_days):
            ...
            if not existing or not existing.get("card_id"):
                recovered = synckey_to_card.get(item.key)
    ...
    logger.info(
        "Sync totals: lists_created=%s cards_created=%s cards_updated=%s cards_adopted=%s canvas_done_skipped=%s",
        lists_created,
        cards_created,
        cards_updated,
        cards_adopted,

...
def _extract_synckey(desc: str) -> Optional[str]:
    if not desc:
        return None
    for line in str(desc).splitlines():
        line = line.strip()
        if line.startswith("SyncKey="):
            return line.split("=", 1)[1].strip()
    return None

def _extract_canvas_url(desc: str) -> Optional[str]:
    if not desc:
        return None
    for line in str(desc).splitlines():
        line = line.strip()
        if line.lower().startswith("canvas link:"):
            return line.split(":", 1)[1].strip()
    return None

def _ensure_synckey_in_desc(desc: str, *, synckey: str) -> str:
    if _extract_synckey(desc):
        return desc

    lines = str(desc or "").splitlines()
    out: list[str] = []
    inserted = False
    for line in lines:
        if not inserted and line.strip() == "(Autogenerated from Canvas)":
            out.append(f"SyncKey={synckey}")
            inserted = True
        out.append(line)
    if not inserted:
        if out and out[-1].strip() != "":
            out.append("")
        out.append(f"SyncKey={synckey}")
        out.append("(Autogenerated from Canvas)")
    return "\n".join(out)

def _bootstrap_state_from_board(
    *,
    state: SyncState,
    existing_lists: BoardLists,
    board_cards: list[Dict[str, Any]],
    synckey_to_card: Dict[str, Dict[str, Any]],
) -> None:
    course_list_votes: Dict[str, Counter[str]] = defaultdict(Counter)
    for key, c in synckey_to_card.items():
        parts = str(key).split(":")
        if len(parts) != 3:
            continue
        _kind, course_id_str, _item_id_str = parts
        lid = str(c.get("idList") or "")
        if course_id_str and lid:
            course_list_votes[course_id_str][lid] += 1

    for course_id_str, counter in course_list_votes.items():
        if not counter:
            continue
        list_id, _count = counter.most_common(1)[0]
        state.course_to_list[course_id_str] = list_id
    ...
    for key, c in synckey_to_card.items():
        ...
        if not cid:
            continue
        state.item_to_card[str(key)] = {
            "card_id": cid,
            "checksum": "",
            "status": "active",
            "locked": False,
            "origin_list_id": lid,
            "last_seen_list_id": lid,
            "rendered_name": str(c.get("name") or ""),
            "rendered_desc": str(c.get("desc") or ""),
            "rendered_due": str(c.get("due") or ""),
        }

    # Recover well-known lists by name (prevents duplicate list creation).
    token_list_id = existing_lists.by_name.get("Canvas Token") or ""
    todo_list_id = existing_lists.by_name.get("TODO") or ""
    done_list_id = existing_lists.by_name.get("Done") or ""
    if token_list_id:
        state.meta["token_list_id"] = token_list_id
    if todo_list_id:
        state.meta["todo_list_id"] = todo_list_id
    if done_list_id:
        state.meta["done_list_id"] = done_list_id

    # Recover well-known cards (prevents duplicate meta cards).
    if token_list_id:
        for c in board_cards:
            if str(c.get("idList") or "") != token_list_id:
                continue
            desc = str(c.get("desc") or "")
            name = str(c.get("name") or "")
            if "Last successful sync run" in desc or name.startswith("Last synced:") or name == "Last Sync":
                state.meta["last_sync_card_id"] = str(c.get("id") or "")
            if (
                "Tracks when your Canvas access token expires" in desc
                or ("Lifetime:" in desc and "Expires:" in desc and "(Autogenerated)" in desc)
            ):
                state.meta["token_card_id"] = str(c.get("id") or "")

    # Recover course info cards by list (best effort).
    list_to_course_ids: Dict[str, str] = {}
    for course_id_str, list_id in state.course_to_list.items():
        if list_id:
            list_to_course_ids[list_id] = course_id_str

    for c in board_cards:
        name = str(c.get("name") or "")
        if not name.endswith("Class Info"):
            continue
        desc = str(c.get("desc") or "")
        if "(Autogenerated from Canvas)" not in desc:
            continue
        lid = str(c.get("idList") or "")
        course_id_str = list_to_course_ids.get(lid)
        if course_id_str and course_id_str not in state.course_info_card:
            state.course_info_card[course_id_str] = str(c.get("id") or "")
...
class DedupeSummary:
    ...
    groups_deduped: int
    cards_archived: int
    cards_skipped_manual: int


def ensure_state_board(*, state: SyncState, board_id: str) -> None:
    stored = str(state.meta.get("board_id") or "").strip()
    if stored and stored != board_id:
        msg = f"State file board_id={stored} does not match requested board_id={board_id}."
        abort = os.getenv("SYNC_ABORT_ON_BOARD_MISMATCH", "0").strip().lower() in {
            "1",
            "true",
            "yes",
            "y",
    ...
    state.meta["board_id"] = board_id


def _state_has_payload(state: SyncState) -> bool:
    meta = dict(state.meta or {})
    meta.pop("board_id", None)
    return bool(
        state.course_to_list
        or state.item_to_card
        or state.managed_list_ids
        or state.course_info_card
        or meta
    )
...
def _build_desc(course_name: str, item: CanvasItem) -> str:
    ...
    lines.append("")
    lines.append(f"SyncKey={item.key}")
...
def _parse_activity(activity_iso: Optional[str]) -> datetime:
    if not activity_iso:
        return datetime.min.replace(tzinfo=timezone.utc)
    try:
        return datetime.fromisoformat(activity_iso.replace("Z", "+00:00")).astimezone(timezone.utc)
    except Exception:
        return datetime.min.replace(tzinfo=timezone.utc)
...
def _card_is_manual(card: Dict[str, Any], state_entry: Optional[Dict[str, Any]]) -> bool:
    if not state_entry:
        desc = str(card.get("desc") or "")
        return "(Autogenerated from Canvas)" not in desc
    if state_entry.get("locked") is True:
        return True
    if str(state_entry.get("status") or "") in {"manual", "done"}:
        return True
    origin = str(state_entry.get("origin_list_id") or "")
    if origin and str(card.get("idList") or "") != origin:
        return True
    rendered_name = str(state_entry.get("rendered_name") or "")
    rendered_desc = str(state_entry.get("rendered_desc") or "")
    rendered_due = str(state_entry.get("rendered_due") or "")
    if rendered_name or rendered_desc or rendered_due:
        if _norm_text(str(card.get("name") or "")) != _norm_text(rendered_name):
            return True
        if _norm_text(str(card.get("desc") or "")) != _norm_text(rendered_desc):
            return True
        if not _due_equal(str(card.get("due") or ""), rendered_due):
            return True
    return False
...
def _pick_keep_card(
    ...
    *,
    ...
    state: SyncState,
    state_by_card: Dict[str, Dict[str, Any]],
) -> Tuple[Dict[str, Any], List[Dict[str, Any]], List[Dict[str, Any]]]:
    def _activity(card: Dict[str, Any]) -> datetime:
        return _parse_activity(str(card.get("dateLastActivity") or ""))

    manual = []
    non_manual = []
    for card in cards:
        entry = state_by_card.get(str(card.get("id") or ""))
        if _card_is_manual(card, entry):
            manual.append(card)
        else:
            non_manual.append(card)

    if manual:
        keep = max(manual, key=_activity)
        to_archive = non_manual
        skipped_manual = [c for c in manual if str(c.get("id") or "") != str(keep.get("id") or "")]
        return keep, to_archive, skipped_manual

    keep = None
    if synckey:
        entry = state.item_to_card.get(synckey)
        if entry:
            state_card_id = str(entry.get("card_id") or "")
            for card in cards:
                if str(card.get("id") or "") == state_card_id:
                    keep = card
                    break
    ...
    if keep is None:
        ...
        for card in cards:
            ...
            if not entry:
                continue
            origin = str(entry.get("origin_list_id") or "")
            if origin and origin == str(card.get("idList") or ""):
                origin_matches.append(card)
        if origin_matches:
            keep = max(origin_matches, key=_activity)

    if keep is None:
        keep = max(cards, key=_activity)

    keep_id = str(keep.get("id") or "")
    to_archive = [c for c in cards if str(c.get("id") or "") != keep_id]
...
def dedupe_board(
    *,
    ...
    board_cards = trello.get_board_cards(
        board_id,
        filter="open",
        fields="id,desc,closed,idList,due,name,dateLastActivity",
    )

    groups: Dict[str, List[Dict[str, Any]]] = {}
    for card in board_cards:
        desc = str(card.get("desc") or "")
        synckey = _extract_synckey(desc)
        if synckey:
            group_key = f"synckey:{synckey}"
        else:
            url = _extract_canvas_url(desc)
            if not url:
                continue
            group_key = f"url:{url}"
    ...
    state_by_card = {
        str(v.get("card_id") or ""): v
        for v in state.item_to_card.values()
        if isinstance(v, dict) and v.get("card_id")
    }

    groups_scanned = len(groups)
    groups_deduped = 0
    cards_archived = 0
    cards_skipped_manual = 0
    archived_ids: set[str] = set()

    for group_key, cards in groups.items():
        if len(cards) < 2:
            continue
        synckey = group_key.split(":", 1)[1] if group_key.startswith("synckey:") else None
        keep, to_archive, skipped_manual = _pick_keep_card(
            cards,
            synckey=synckey,
            state=state,
        ...
        keep_id = str(keep.get("id") or "")
        archive_ids = [str(c.get("id") or "") for c in to_archive if str(c.get("id") or "")]
        skipped_ids = [str(c.get("id") or "") for c in skipped_manual if str(c.get("id") or "")]

        logger.info(
            "Dedupe group=%s keep=%s archive=%s manual_skipped=%s",
            group_key,
            keep_id,
            ",".join(archive_ids) if archive_ids else "(none)",
        ...
        if to_archive:
            ...
            if not dry_run:
                for c in to_archive:
                    ...
                    if not cid:
                        continue
                    trello.archive_card(cid)
                    archived_ids.add(cid)
                    cards_archived += 1
            else:
                cards_archived += len(to_archive)

        if skipped_manual:
            cards_skipped_manual += len(skipped_manual)

        if not dry_run and synckey:
            entry = state.item_to_card.get(synckey)
            if not isinstance(entry, dict):
                entry = {}
                state.item_to_card[synckey] = entry
            entry.setdefault("checksum", "")
            entry.setdefault("status", "active")
            entry.setdefault("locked", False)
            entry["card_id"] = keep_id
            if not entry.get("origin_list_id"):
                entry["origin_list_id"] = str(keep.get("idList") or "")
            entry["last_seen_list_id"] = str(keep.get("idList") or "")
            if not entry.get("rendered_name"):
                entry["rendered_name"] = str(keep.get("name") or "")
            if not entry.get("rendered_desc"):
                entry["rendered_desc"] = str(keep.get("desc") or "")
            if not entry.get("rendered_due"):
                entry["rendered_due"] = str(keep.get("due") or "")

    if not dry_run and archived_ids:
        state.item_to_card = {
            k: v
            for (k, v) in state.item_to_card.items()
            if str(v.get("card_id") or "") not in archived_ids
        }

    return DedupeSummary(
        groups_scanned=groups_scanned,
        groups_deduped=groups_deduped,
        cards_archived=cards_archived,
        cards_skipped_manual=cards_skipped_manual,
    )
...
def sync_once(
    *,
    ...
    # If state is missing or stale, recover mappings by scanning existing cards.
    synckey_to_card: Dict[str, Dict[str, Any]] = {}
    url_to_card: Dict[str, Dict[str, Any]] = {}
    board_cards: list[Dict[str, Any]] = []
    try:
        board_cards = trello.get_board_cards(board_id, filter="open", fields="id,desc,closed,idList,due,name")
        for c in board_cards:
            key = _extract_synckey(str(c.get("desc") or ""))
            if key:
                synckey_to_card[key] = c
            url = _extract_canvas_url(str(c.get("desc") or ""))
            if url and url not in url_to_card:
                url_to_card[url] = c
    except Exception:
        synckey_to_card = {}
        url_to_card = {}
    ...
    bootstrap_state = os.getenv("SYNC_BOOTSTRAP_STATE_FROM_BOARD", "0").strip().lower() in {
        "1",
        "true",
        "yes",
        "y",
    ...
    state_looks_empty = not _state_has_payload(state)
    if bootstrap_state and state_looks_empty and synckey_to_card:
        _bootstrap_state_from_board(
            state=state,
            existing_lists=existing_lists,
            board_cards=board_cards,
            synckey_to_card=synckey_to_card,
        )
        logger.warning(
            "Bootstrapped missing state from existing Trello cards: courses=%s items=%s",
    ...
    if abort_on_missing_state and state_looks_empty and (synckey_to_card or url_to_card):
        ...
        for k, c in list(synckey_to_card.items())[:5]:
            cid = str(c.get("id") or "")
            lid = str(c.get("idList") or "")
            lname = existing_lists.by_id.get(lid, "")
            examples.append(f"{k} -> card={cid} list={lid}{' (' + lname + ')' if lname else ''}")
        if not examples:
    ...
    cards_updated = 0
    cards_adopted = 0
    cards_skipped_canvas_done = 0
    cards_synckey_patched = 0
    ...
    for course in courses:
        ...
        for item in canvas.upcoming_items(course_id, within_days=due_within_days):
            ...
            if not existing or not existing.get("card_id"):
                recovered = synckey_to_card.get(item.key) or url_to_card.get(item.url or "")
                if recovered and recovered.get("id"):
                    card_id = str(recovered["id"])
                    recovered_desc = str(recovered.get("desc") or "")
                    patched_desc = _ensure_synckey_in_desc(recovered_desc, synckey=item.key)
                    if patched_desc != recovered_desc and "(Autogenerated from Canvas)" in recovered_desc:
                        trello.update_card(
                            card_id,
                            str(recovered.get("name") or title),
                            patched_desc,
                            str(recovered.get("due") or ""),
                        )
                        recovered["desc"] = patched_desc
                        cards_synckey_patched += 1
                    state.item_to_card[item.key] = {
                        ...
                        "last_seen_list_id": str(recovered.get("idList") or list_id),
                        "rendered_name": str(recovered.get("name") or title),
                        "rendered_desc": str(recovered.get("desc") or desc),
                        "rendered_due": str(recovered.get("due") or item.due_iso or ""),
                    }
                    existing = state.item_to_card[item.key]
                    cards_adopted += 1
                else:
    ...
    logger.info(
        "Sync totals: lists_created=%s cards_created=%s cards_updated=%s cards_adopted=%s synckey_patched=%s canvas_done_skipped=%s",
        lists_created,
        cards_created,
        cards_updated,
        cards_adopted,
        cards_synckey_patched,
        cards_skipped_canvas_done,
    )
